 ...
 *
 * @author Kacper Rowinski <krowinski@implix.com>, Grzegorz Struczynski <gstruczynski@implix.com>
 * http://implix.com
 *
...
class JsonRpcClient
{
	...
	public function __call($method, $params)
	{
		...
		$validate_params = array
		(
			$response['id'] != $request_id => 'Request id: '.$request_id.' is different from Response id: '.$response['id'],
	 ...
	 * @return void
	 */
	protected function checkForErrors(& $validate_array)
	{
		foreach ($validate_array as $test => $error)
		{
			if ($test)
	...
	protected function debug($add, $show = false)
	{
		...
		$debug .= $add;
		// get starttime
		$start_time = empty($start_time) ? array_sum(explode(' ', microtime())) : $start_time;
		if (true === $show && !empty($debug))
		{
			// get endtime
			$end_time = array_sum(explode(' ', microtime()));
			// performance summary
			$debug .= 'Request time: '.round($end_time - $start_time, 3).' s Memory usage: '.round(memory_get_usage() / 1024)." kb\r\n";
			echo nl2br($debug);
			// send output imidiately
			flush();
			// clean static
			$debug = $start_time = null;
		}
	}
}
?>